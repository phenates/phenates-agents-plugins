---
name: code-explain
description: Code explanation and analysis — turns complex code, algorithms, or design patterns into clear narratives, visual diagrams (Mermaid flowcharts/class diagrams), step-by-step breakdowns, and learning resources. Use this skill whenever the user asks to explain, understand, break down, walk through, or teach a piece of code, an algorithm, a design pattern, or how a system works — even if they just paste a code block and ask "what does this do?" or "how does this work?".
...
Use whatever the user specified as the request/arguments for this task (the code or file to explain, the audience, the angle they care about, etc.) to scope the work below.

## Before generating anything: clarify scope

An explanation that covers everything — complexity metrics, diagrams, step-by-step breakdown, interactive examples, pitfalls, best practices, a learning path, and practice exercises — is a lot to hand someone who just wanted to understand one function. Unless the request already makes this clear (e.g. "explain this regex," "what does this recursive function do," a narrow snippet with an obvious single question), ask before generating:

1. **Quel niveau de profondeur ?** — résumé rapide (ce que fait le code, en quelques phrases), explication détaillée (étape par étape), ou approfondissement complet (diagrammes, pièges, bonnes pratiques, parcours d'apprentissage) ?
2. **Quel est le scope ?** — une fonction/un bloc précis, un fichier, ou un système/flux complet ?
3. **Pour quel public ?** — débutant (analogies, vocabulaire simple), développeur expérimenté qui découvre juste ce code, ou revue technique entre pairs ?
4. **Format utile ici ?** — texte seul, diagrammes Mermaid, exemples exécutables, ou une combinaison ?

Skip this step when the request is already unambiguous — a short pasted snippet with a direct question ("why does this throw?", "what does this decorator do?") doesn't need an interview, just answer it. Ask when the target is a whole file, module, or "explain this codebase" — that's where scope and depth genuinely change the size of the output.
...
Scale the output to the depth agreed in the scoping step above — don't include every section below by default:
