...
Documentation generation is expensive to redo and easy to over-scope — left unconstrained, it's tempting to document the entire codebase at maximum detail, which produces bloated output nobody asked for and buries the parts the user actually cares about. Unless the user's request already answers these clearly (e.g. they named a specific file, a specific doc type, and a clear scope), ask before generating:

1. **Types de documentation** — API, architecture, code (docstrings/README), guide utilisateur, développeur, ou tout ?
2. **Scope** — tout le projet, un dossier/module précis, ou une fonctionnalité spécifique ?
3. **Format de sortie** — Markdown, OpenAPI/Swagger, diagrammes Mermaid, ou une combinaison ?
4. **Niveau de détail** — vue d'ensemble haut niveau, documentation exhaustive (chaque fonction), ou un équilibre ?

Skip this step only when the request is already unambiguous — e.g. "add a docstring to this function," "write a README for this repo" (scope and type are self-evident), or a follow-up in an already-scoped conversation (rerunning on the same target with the same format already agreed). When in doubt, ask: a scoped request that reruns once is cheaper than an unscoped one that has to be redone.
